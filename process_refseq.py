...
for line in f:
    ...
    for i in range(exonCount-1):
        exon = exon_pairs[i]
        exonic_newline = chro+'\t'+exon[0]+'\t'+exon[1]+'\t'+gene_ID+'\t'+str(i+1)+'\t'+strand+'\n'
        exonic.write(exonic_newline)

        intron = intron_pairs[i]
        intronic_newline = chro+'\t'+intron[0]+'\t'+intron[1]+'\t'+gene_ID+'\t'+str(i+1)+'\t'+strand+'\n'
        intronic.write(intronic_newline)
    exon = exon_pairs[exonCount-1]
    exonic_newline = chro+'\t'+exon[0]+'\t'+exon[1]+'\t'+gene_ID+'\t'+str(exonCount)+'\t'+strand+'\n'
    exonic.write(exonic_newline)
