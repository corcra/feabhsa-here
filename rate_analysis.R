...
# --- get rates --- #
early<-get_rate(five,twelve,7.5)
mid<-get_rate(twelve,twentyfive,12.5)
late<-get_rate(twentyfive,fifty,25)

# --- combine the covariates -- #
early<-add_covars(early,n_gb,int1len,n_exon)
mid<-add_covars(mid,n_gb,int1len,n_exon)
late<-add_covars(late,n_gb,int1len,n_exon)
